...
MIN_QUALITY_VALUE = .1 # if cell expresses quality score lower than this amount, set to this amount
...
merged_ephys_gene_corrs = lapply(names(patch_seq_datasets), function(dataset_name){
  ...
  # weights = 1/(lapply(joined_df$contam_sum[use_dataset_inds], function(x) max(x, MIN_QUALITY_VALUE)) %>% unlist)
