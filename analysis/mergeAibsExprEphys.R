...
upper_regions_expr = c('L1', 'L1-L2/3', 'L2/3', 'upper', 'L4')
lower_regions_expr = c('L5', 'L6', 'L6a', 'L6b', 'lower')
...
aibs_expr_filt_stats = tasic_expr %>%
  group_by(sample.name, layer_coarse, broad_type) %>% select(Hcn3, broad_type) %>% 
...
tasic_expr_cre = tasic_expr %>% group_by(sample.name, layer_coarse, broad_type) %>% summarize_at(aibs_gene_names, mean)
...
upper_regions = c('VISp1', 'VISp2/3', 'VISp4')
lower_regions = c('VISp5', 'VISp6a', 'VISp6b')
...
aibsEphys = aibsEphys %>% filter(brain_region %in% primary_vis_regions) %>% 
  mutate('layer_coarse' = case_when((.$brain_region %in% upper_regions) & 
                                      ...
                                      (.$sample.name %in% lines_split)  ~ 'lower',
                                     TRUE ~ "all"),
         'broad_type' = case_when(.$spines %in% c('sparsely spiny', 'spiny') ~ 'Glutamatergic Neuron',
                                  .$spines %in% c('aspiny') ~ 'GABA-ergic Neuron'))
MIN_CELLS = 1
temp_ephys_names = response_vars[response_vars %in% colnames(aibsEphys)]

aibsEphysFilt = aibsEphys %>% group_by(sample.name, layer_coarse, broad_type) %>%
...
aibs_ephys_filt_stats = aibsEphys %>% 
  filter(reporter == 'positive') %>%
  group_by(sample.name, layer_coarse, broad_type) %>% mutate(num_cells_ephys = n()) %>% filter(num_cells_ephys >= MIN_CELLS) %>% select(num_cells_ephys, colors) %>% distinct(.keep_all = T)


## join aibs ephys and expression data
aibs_cre_joined = merge(tasic_expr_cre, aibs_ephys_cre, by = c("sample.name", "layer_coarse", "broad_type"), all = F, unique = T)
...
aibs_cre_joined = left_join(aibs_cre_joined, aibs_expr_filt_stats , by = c("sample.name", "layer_coarse", "broad_type")) 
aibs_cre_joined = left_join(aibs_cre_joined, aibs_ephys_filt_stats %>% select(-colors), by = c("sample.name", "layer_coarse", "broad_type")) 
...
# remove samples with too few cells for either ephys or expr
aibs_cre_joined = aibs_cre_joined %>% filter(num_cells_expr > 2, num_cells_ephys > 1)

# unite 
new_sample_names = aibs_cre_joined %>% select(one_of('sample.name', 'layer_coarse', 'broad_type')) %>% unite(col = sample.name, sample.name, layer_coarse, broad_type) %>% select(sample.name)
